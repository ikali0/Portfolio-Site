...
const Gallery4 = ({
  ...
  return <section className="py-8 md:py-12 overflow-hidden px-[10px] lg:py-[8px]">
      ...
      <div className="relative w-full min-w-0 overflow-hidden" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{
      ...
      WebkitMaskImage: 'linear-gradient(to right, black 0%, black 85%, transparent 100%)'
        ...
        <Carousel setApi={setCarouselApi} plugins={[autoplayPlugin()]} opts={{
        loop: true,
        align: "start",
        breakpoints: {
          "(max-width: 768px)": {
          ...
          <CarouselContent className="-ml-3 md:-ml-4 px-4 md:px-6 lg:px-8">
            {items.map((item, index) => <CarouselItem key={item.id} className="pl-3 md:pl-4 basis-[85%] sm:basis-[75%] md:basis-1/2 lg:basis-1/3">
                <motion.button onClick={e => handleCardClick(item, e)} className="group rounded-xl block text-left w-full cursor-pointer min-w-0" initial={{
              opacity: 0,
              y: 20
            }} whileInView={{
              opacity: 1,
              ...
              scale: 0.98
                  ...
                  <div className="group relative w-full min-w-0 aspect-[4/5] sm:aspect-[5/4] overflow-hidden rounded-xl">
                    <img src={item.image} alt={item.title} className="absolute inset-0 w-full h-auto min-h-full object-cover object-center transition-transform duration-300 group-hover:scale-105" />
        ...
        <div className="mt-4 flex justify-center gap-1.5 sm:mt-6 sm:gap-2 md:mt-8">
          {items.map((_, index) => <button key={index} className={`h-1.5 w-1.5 rounded-full transition-colors sm:h-2 sm:w-2 ${currentSlide === index ? "bg-primary" : "bg-primary/20"}`} onClick={() => carouselApi?.scrollTo(index)} aria-label={`Go to slide ${index + 1}`} />)}
        </div>
      </div>
          ...
          {selectedItem && <>
              ...
              <div className="mt-3 space-y-4 sm:mt-4 sm:space-y-5 md:space-y-6">
                ...
                <div className="rounded-lg overflow-hidden aspect-video min-w-0">
                  <img src={selectedItem.image} alt={selectedItem.title} className="w-full h-auto min-h-full object-cover" />
                </div>

                {/* Case Study Details */}
                <div className="grid gap-4 sm:gap-5 md:gap-6">
                  {selectedItem.problem && <div className="space-y-1.5 sm:space-y-2">
                  ...
                  {selectedItem.approach && <div className="space-y-1.5 sm:space-y-2">
                      <h4 className="text-sm font-semibold text-foreground flex items-center gap-2 sm:text-base">
                  ...
                  {selectedItem.outcome && <div className="space-y-1.5 sm:space-y-2">
                      <h4 className="text-sm font-semibold text-foreground flex items-center gap-2 sm:text-base">
                  ...
                  {selectedItem.techStack && selectedItem.techStack.length > 0 && <div className="space-y-1.5 sm:space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1.5 pl-3.5 sm:gap-2 sm:pl-4">
                        {selectedItem.techStack.map(tech => <span key={tech} className="px-2 py-0.5 text-[10px] rounded-full bg-muted text-muted-foreground sm:px-3 sm:py-1 sm:text-xs">
                            {tech}
                          </span>)}
                      </div>
                    </div>}
                ...
                {/* CTA */}
                {selectedItem.href && selectedItem.href !== "#" && <div className="pt-3 border-t sm:pt-4">
                    <a href={selectedItem.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-medium text-primary hover:underline sm:gap-2 sm:text-sm">
