...
import { faAtom, faBrain, faShieldHalved, faBalanceScale, faChevronLeft, faChevronRight, faExternalLinkAlt } from "@fortawesome/free-solid-svg-icons";
...
const categories: Category[] = [{
  ...
  icon: faAtom,
  color: "text-neural",
  bgColor: "bg-neural/10 border-neural/30"
}];
const categoryProjects: CategoryProject[] = [{
  ...
  description: "ML-powered smart grid efficiency modeling",
  image: ethicsDashboard,
  categories: ["energy"]
}, {
  id: "quantum-sim",
  title: "Quantum Circuit Simulator",
  description: "Educational quantum computing visualization tool",
  image: biasDetection,
  categories: ["quantum"]
}];
export function HeroCategoryCarousel() {
  ...
  const filteredProjects = selectedCategory ? categoryProjects.filter(p => p.categories.includes(selectedCategory)) : [];
  ...
  return <div className="w-full">
      ...
      <div className="relative">
        ...
        <button onClick={scrollNext} disabled={!canScrollNext} className={cn("absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full", "bg-card/80 backdrop-blur-sm border border-border/50 shadow-sm", "flex items-center justify-center transition-all duration-200", "hover:bg-card hover:border-border", "disabled:opacity-0 disabled:pointer-events-none", "hidden sm:flex")} aria-label="Scroll right">
      ...
      <AnimatePresence mode="wait">
        {selectedCategory && filteredProjects.length > 0 && <motion.div initial={{
        opacity: 0,
        height: 0
      }} animate={{
        opacity: 1,
        ...
        ease: "easeInOut"
            ...
            <div className="p-4 rounded-xl bg-card/60 backdrop-blur-sm border border-border/50 shadow-lg">
              ...
              transition: {
                duration: 0.2
                    ...
                    <div className="aspect-[16/10] overflow-hidden">
                      <img src={project.image} alt={project.title} loading="lazy" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                    </div>
                    ...
                    if (!cat) return null;
                    return <span key={catId} className={cn("text-[10px] px-1.5 py-0.5 rounded-full border", cat.bgColor, cat.color)}>
                              {cat.label}
                            </span>;
                  })}
                      </div>
                      ...
                      {/* View Link */}
                      {project.link && <a href={project.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-2 text-xs font-medium text-primary hover:underline">
                          View Project
              ...
              <div className="mt-4 text-center">
                <a href="#portfolio" className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline">
                  View all projects
