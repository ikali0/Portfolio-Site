...
const IconBubble = ({
  ...
  return (
    <div
      className={`${sizeClasses[size]} ${variantClasses[variant]} rounded-full border shadow-sm flex items-center justify-center backdrop-blur-sm`}
    >
...
const FloatingBadge = ({
  ...
  return (
    <span
      className={`${variantClasses[variant]} text-[10px] font-medium px-2.5 py-1 rounded-full border shadow-sm backdrop-blur-sm whitespace-nowrap`}
    >
      {children}
    </span>
  );
...
const Hero = () => {
  return (
    <section className="relative min-h-[100svh] flex items-center justify-center overflow-hidden px-6 pt-28 pb-20 bg-background">
      {/* Subtle curved line decorations */}
      <svg
        className="absolute inset-0 w-full h-full pointer-events-none opacity-[0.08] dark:opacity-[0.04]"
      ...
      <div className="relative z-20 w-full max-w-2xl mx-auto flex flex-col items-center text-center">
        ...
        >
          <a
            href="#contact"
            className="inline-flex items-center gap-2 px-6 py-3 bg-foreground text-background font-medium rounded-lg hover:bg-foreground/90 transition-colors shadow-sm"
          >
            Get in touch
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <a
                  href="https://linkedin.com/in/ingakali"
                  target="_blank"
