  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 w-full max-w-3xl mx-auto flex flex-col items-center text-center min-w-0">
        <motion.span
          {...fadeUp(0)}
          className="inline-block text-[9px] sm:text-[10px] uppercase tracking-widest py-1 px-3 rounded-full backdrop-blur-md border border-purple-300/60 dark:border-purple-700/40 mb-4 sm:mb-6 bg-purple-100/70 dark:bg-purple-900/70 text-purple-800 dark:text-purple-200 shadow-sm"
        >
        ...
        <motion.div {...fadeUp(0.08)} className="relative mb-3 sm:mb-5">
          ...
          <h1
            aria-hidden
            className="absolute inset-0 text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight bg-gradient-to-r from-purple-600/20 via-teal-500/20 to-lime-500/20 bg-clip-text text-transparent blur-md -z-10"
        ...
        <motion.p
          {...fadeUp(0.18)}
          className="text-sm sm:text-sm md:text-base lg:text-lg text-muted-foreground max-w-[min(90vw,640px)] leading-relaxed mb-6 sm:mb-10 px-1"
        >
          I translate{" "}
        ...
        <TooltipProvider>
          <motion.div
            {...fadeUp(0.26)}
            className="flex flex-wrap gap-2 sm:gap-3 mb-10 sm:mb-14 justify-center"
          >
            {SOCIAL_LINKS.map(
                ...
                <Tooltip key={label}>
                  <TooltipTrigger asChild>
                    <motion.a
                      ...
                      aria-label={label}
                      {...hoverMotion}
                      className={`flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-background/80 backdrop-blur-md border-2 ${borderClass} transition-all shadow-sm hover:shadow-md`}
                    >
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}>
          ...
          >
            <span className="text-[9px] sm:text-[10px] uppercase tracking-widest mb-2 sm:mb-3">
              Explore
            </span>
            {!reduceMotion && (
              <motion.div
