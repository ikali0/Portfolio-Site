...
function ProjectCardFront({ project }: { project: ProjectData }) {
  return (
    <div className="flex flex-col h-full w-full overflow-hidden rounded-md bg-card">
      ...
      <div className="absolute left-0 top-0 bottom-0 w-3 md:w-4 bg-muted/60 border-r border-border flex flex-col justify-around py-2">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-border mx-auto" />
        ))}
      </div>
      ...
      <div className="ml-3 md:ml-4 flex flex-col h-full">
        ...
        <div 
          className="flex-1 p-1.5 md:p-2 relative"
          style={{
        ...
        >
          ...
          <div className="flex flex-wrap gap-0.5 mt-1">
            {project.tags.map(tag => (
              <span 
                key={tag} 
                className="text-[6px] md:text-[8px] text-primary bg-primary/10 px-1 py-0.5 rounded border border-primary/20"
              >
                {tag}
              </span>
            ))}
...
function ProjectCardBack({ project }: { project: ProjectData }) {
  return (
    <div 
      className="flex flex-col h-full w-full rounded-md bg-card relative overflow-hidden"
    >
      {/* Notebook spiral binding effect */}
      ...
      {/* Content */}
      <div 
        className="ml-3 md:ml-4 flex-1 p-2 md:p-3 flex flex-col justify-center items-center text-center"
        style={{
          backgroundImage: 'repeating-linear-gradient(transparent, transparent 11px, hsl(var(--border) / 0.3) 11px, hsl(var(--border) / 0.3) 12px)',
          backgroundSize: '100% 12px'
        }}
      >
        <p className="text-[7px] md:text-[9px] text-muted-foreground leading-relaxed line-clamp-5 mb-2">
          {project.description}
        </p>
        {project.live && (
          <a 
            ...
            className="inline-flex items-center gap-1 bg-primary text-primary-foreground px-2 py-0.5 rounded text-[7px] md:text-[8px] hover:bg-primary/90 transition-colors shadow-sm"
          >
...
const Portfolio = () => {
  return (
    <section id="portfolio" className="py-10 md:py-16 px-3 md:px-4 bg-card/50">
      <div className="container mx-auto max-w-3xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3 justify-items-center">
          {projects.map(project => (
            <FlippingCard 
              key={project.title} 
              width={140} 
              height={150} 
              className="w-full max-w-[140px] md:max-w-[170px] md:!w-[170px] md:!h-[175px]" 
