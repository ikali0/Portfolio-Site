...
function ProjectCard({
  ...
  return <article className="portfolio-card-3d h-full" style={{
    animationDelay: `${index * 100}ms`
      ...
      <div className="portfolio-card-inner flex flex-col h-full overflow-hidden rounded-xl bg-card border-2 border-border">
        ...
        <div className="card-image-shine relative aspect-[16/10] sm:aspect-[4/3] overflow-hidden bg-muted">
          <img src={project.image} alt={project.title} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 ease-out" />
          <div className="absolute inset-0 bg-gradient-to-t from-card/40 via-transparent to-transparent opacity-65" aria-hidden="true" />
        </div>

        {/* Content */}
        <div className="relative flex flex-col flex-1 p-card">
          <h3 className="text-subheading font-display font-semibold text-foreground leading-tight mb-2 line-clamp-2">
            {project.title}
          </h3>
          ...
          {/* Action Button */}
          {project.live && <a href={project.live} target="_blank" rel="noopener noreferrer" className="portfolio-button inline-flex items-center justify-center gap-2 w-full py-3 px-4 text-body-sm font-medium text-primary-foreground bg-primary focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2 focus:ring-offset-card rounded-sm" aria-label={`View ${project.title} project`}>
...
const Portfolio = () => {
  return <section id="portfolio" className="py-section-sm md:py-section px-4 bg-muted/30" aria-labelledby="portfolio-heading">
      <div className="container mx-auto max-w-6xl">
        <SectionHeader overline="Personal Projects" title="Featured Work" description="A selection of projects I've designed and built." />

        {/* Responsive Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-container md:gap-container-lg" role="list" aria-label="Portfolio projects">
