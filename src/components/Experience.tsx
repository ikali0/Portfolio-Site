...
const Experience = () => {
  ...
  return (
    <section id="experience" className="py-10 md:py-16 px-3 md:px-4 bg-card/30">
      <div className="container mx-auto max-w-3xl">
        ...
        <div className="relative">
          {/* Timeline Line */}
          <div 
            className="absolute left-2 md:left-1/2 top-0 bottom-0 w-0.5 md:-translate-x-1/2"
            style={{
          ...
          <div className="space-y-3 md:space-y-4">
            {experiences.map((exp, index) => (
              ...
              >
                {/* Timeline Node */}
                <motion.div
                  className={`absolute left-0.5 md:left-1/2 top-2.5 w-3 h-3 rounded-full md:-translate-x-1/2 z-10 ${getStatusStyles(exp.status)}`}
                  style={{
                ...
                {/* Connector Line */}
                <div 
                  className={`hidden md:block absolute top-3.5 w-4 h-px ${
                ...
                <div className={`md:w-[calc(50%-1rem)] ${index % 2 === 0 ? 'md:ml-auto md:pl-4' : 'md:mr-auto md:pr-4'}`}>
                  {/* Notebook-style Card */}
                  <motion.div
                    className="relative bg-card border border-border rounded-md overflow-hidden cursor-pointer"
                    style={{
                  ...
                  >
                    ...
                    <div className="absolute left-0 top-0 bottom-0 w-2.5 md:w-3 bg-muted/50 border-r border-border/60 flex flex-col justify-around py-1.5">
                      {[...Array(4)].map((_, i) => (
                        <div key={i} className="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-border mx-auto" />
                      ))}
                    </div>

                    {/* Content with notebook lines */}
                    <div 
                      className="ml-2.5 md:ml-3 p-2 md:p-3"
                      style={{
                    ...
                    >
                      ...
                      {/* Expand Button */}
                      <button 
                        className="flex items-center gap-0.5 text-[8px] md:text-[9px] text-primary hover:text-primary/80 transition-colors mb-1.5"
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleExpand(index);
                        }}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {exp.tags.map(tag => (
                          <span 
                            key={tag} 
                            className="px-1 py-0.5 bg-muted text-muted-foreground text-[7px] md:text-[8px] rounded border border-border/40"
                          >
                            {tag}
                          </span>
                        ))}
