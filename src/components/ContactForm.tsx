...
const checkRateLimit = (): {
  ...
  try {
    ...
    const now = Date.now();
    const validSubmissions = submissions.filter(timestamp => now - timestamp < RATE_LIMIT_WINDOW_MS);
...
const recordSubmission = () => {
  try {
    ...
    const cleaned = submissions.filter(ts => now - ts < RATE_LIMIT_WINDOW_MS);
...
const ContactForm = ({
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className={className}>
      ...
      {/* Buttons & Status */}
      <div className="flex items-center mt-4 gap-[8px] my-[8px]">
        <Button type="submit" disabled={isSending}>
          {isSending ? <>
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              Sending...
            </> : <>
              <Send className="mr-2 h-[10px] w-[10px]" />
              Send message
            </>}
        </Button>
