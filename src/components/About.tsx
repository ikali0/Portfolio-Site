...
function CardFront({
  ...
  return <div className="flex flex-col items-center justify-center h-full w-full p-card-sm text-center">
      ...
      <h4 className="font-display text-body-sm font-medium text-foreground mb-1 leading-tight">{data.title}</h4>
      <p className="text-caption text-muted-foreground leading-snug">{data.description}</p>
    </div>;
}
function CardBack({
  data
  ...
  return <div className="flex flex-col items-center justify-center h-full w-full p-card-sm text-center rounded-md bg-secondary/20">
      <p className="text-caption leading-snug text-foreground">{data.backDescription}</p>
    </div>;
}
const About = () => {
      ...
      <ScrollFade>
        <div className="container relative z-10 mx-auto max-w-2xl glass p-card md:p-container px-[30px] py-[30px] rounded-md shadow-sm opacity-85 border-purple-400">
          ...
          <div className="mb-container md:mb-container-lg">
            <p className="text-overline uppercase text-accent font-semibold mb-element-sm">About Me</p>
            <h2 className="font-display text-display-sm text-foreground mb-element-sm">Building with Purpose</h2>
          </div>
          ...
          <div className="text-body-sm md:text-body space-y-card-sm mb-card leading-relaxed text-primary">
            <p>
              ...
              —focused not on hype, but on real-world consequences. I examine how AI systems shape behavior, power,
              access, and trust once deployed.
            </p>
            <p>
              Through long-form writing on AI ethics, alignment, and accountability, I explore where technology delivers
              value, where it causes subtle harm, and what responsible governance truly requires. I publish
              independently to think critically, openly, and without constraint.
            </p>
            <p>
              ...
              . This site brings together my work and serves as a point of connection for readers, researchers,
              collaborators, and thoughtful technologists worldwide.
            </p>
          </div>
